...
export class FooterlinkComponent implements OnInit, AfterViewInit {
  seoForm: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      const id = params['id'];
      if (id) {
        this.userService.getSeoPages(id).subscribe((res: any) => {
          if (res.status) {
  ...
  onSubmit() {
    ...
    if (this.isUpdate) {
      this.userService.editFooterLink(this.updateId, formData).subscribe((res: any) => {
        if (res.status) {
          this.router.navigate(['/footer-link']);
        } else {
          alert('Error Adding SEO Pages');
        }
      });
    } else {
      this.userService.addFooterLink(formData).subscribe((res: any) => {
        if (res.status) {
          this.router.navigate(['/footer-link']);
        } else {
          alert('Error Adding SEO Pages');
